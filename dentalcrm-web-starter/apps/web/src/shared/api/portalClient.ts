...
export const portalApi = axios.create({
  ...
  headers: {
    ...
    'X-Clinic-Slug': import.meta.env.VITE_CLINIC_SLUG ?? 'clinica-demo',
  },
});
...
portalApi.interceptors.request.use((config) => {
  ...
  const clinicSlug = getActiveClinicSlug() ?? getClinicSlug() ?? import.meta.env.VITE_CLINIC_SLUG ?? 'clinica-demo';
